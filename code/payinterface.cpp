    ...
    ui(new Ui::PayInterface)
{
    ...
    connect(paymentPage, &PayPage::paymentConfirmed,
            [=](){ stackedPages->setCurrentWidget(successPage); });

    connect(paymentPage, &PayPage::paymentCancelled,
            [=](){ stackedPages->setCurrentWidget(failPage); });
