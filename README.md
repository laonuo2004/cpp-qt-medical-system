...
| **刘兆钰** | 文档 / 测试 | 文档攥写、功能测试、UI 测试 |
| **胡艺镭** | **后端 & 架构主力** + PRL | 数据库设计、后端业务逻辑、核心API接口实现、管理员后端(在时间允许的情况下)、CR (Code Review) |
| **周鑫** | **前端 & UI 主力** + SMCL | 患者端/医生端/管理员端所有UI设计与实现、用户交互逻辑 |
...
- **【待讨论】** 我们需要共同设计并确认所有的数据表、字段和它们之间的关系。以下是一个示例，需要进一步完善。
...
gantt
    ...
    section 核心开发
    后端API开发 (胡) :crit, task_backend, after task_design, 60h
    前端UI开发 (周) :task_frontend, after task_design, 60h
